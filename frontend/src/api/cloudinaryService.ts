...
export const cloudinaryService = {
  ...
  getSignedUrl: async (
    ...
    transformation: string = 'sp_auto'
  ): Promise<string> => {
    const response = await api.get('/cloudinary/signed-url', {
      params: { publicId, resourceType, transformation },
    });
  ...
  uploadMedia: async (
    ...
    preFetchedSignature?: CloudinarySignatureResponse,
    type: 'upload' | 'authenticated' = 'upload'
  ) => {
    ...
    const {
      ...
      folder: targetFolder,
      type: targetType,
    } = preFetchedSignature || (await cloudinaryService.getSignature(folder, type));
    ...
    if (targetType && targetType !== 'upload') {
