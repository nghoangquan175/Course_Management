...
export const AchievementsTab: React.FC<AchievementsTabProps> = ({ completedCourses }) => {
  return (
        ...
        {completedCourses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {completedCourses.map((course) => (
              <div key={course.id} className="glass rounded-[2rem] border border-white/10 overflow-hidden group hover:border-green-500/30 transition-all duration-500 hover:shadow-2xl hover:shadow-green-500/10 flex flex-col h-full">
                {/* Image Container */}
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="mt-auto space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button 
                        onClick={() => course.certificateUrl && window.open(course.certificateUrl, '_blank')}
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-500 bg-white/[0.02] rounded-[3rem] border border-dashed border-white/10 p-12 text-center">
            ...
            <p className="max-w-xs text-sm opacity-60">Complete your first course and pass the final exam to earn a verified certificate!</p>
          </div>
        )}
      </div>
    </div>
