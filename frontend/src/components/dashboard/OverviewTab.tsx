...
export const OverviewTab: React.FC<OverviewTabProps> = ({ enrolledCourses, isLoading, onRateClick }) => {
  return (
    <div className="flex-1 overflow-hidden flex flex-col space-y-8">
      {/* Stats */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 shrink-0">
        <StatCard label="Enrolled Courses" value={enrolledCourses.length.toString()} icon={<BookOpen />} />
        <StatCard label="In Progress" value={enrolledCourses.filter((c: any) => c.status !== 'COMPLETED').length.toString()} icon={<Clock />} trend="Active" />
        <StatCard label="Completed" value={enrolledCourses.filter((c: any) => c.status === 'COMPLETED').length.toString()} icon={<Trophy />} trend="Well done!" />
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-500 bg-white/5 rounded-[3rem] border border-dashed border-white/10">
            ...
            <Link to="/" className="px-8 py-3 bg-indigo-500 hover:bg-indigo-400 text-white font-bold rounded-2xl transition-all shadow-xl shadow-indigo-500/20">
              Browse Catalog
            </Link>
          </div>
        )}
