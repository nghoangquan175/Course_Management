        ...
        {results.length > 0 ? (
          <div className="space-y-4">
            {results.map((result) => {
              ...
              return (
                <div key={result.id} className="glass rounded-3xl border border-white/5 p-6 hover:border-white/10 transition-all group">
                  <div className="flex flex-col md:flex-row md:items-center gap-6">
                    {/* Status Icon */}
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${result.isPassed ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                      {result.isPassed ? <CheckCircle2 className="w-8 h-8" /> : <XCircle className="w-8 h-8" />}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider mb-1">
                        ...
                        <span className="text-slate-400">{new Date(result.createdAt).toLocaleDateString()}</span>
                      </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-slate-500 bg-white/[0.02] rounded-[3rem] border border-dashed border-white/10 p-12 text-center">
            ...
            <p className="max-w-xs text-sm opacity-60">Complete lessons and take quizzes to track your performance here!</p>
          </div>
        )}
      </div>
    </div>
