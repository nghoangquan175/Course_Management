          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            ...
            >
              ...
              <p className="text-xl text-slate-400 mb-10 max-w-2xl leading-relaxed">
                Join over 5,000+ students learning from the world's best instructors. 
                Start your journey towards a professional career today.
              </p>
      ...
      <section className="py-20 border-y border-white/5 bg-slate-900/30">
        <div className="container mx-auto px-6 max-w-[1440px]">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-12">
            {[
              { label: 'Active Students', value: '15,000+', icon: <Users className="w-6 h-6 text-indigo-400" /> },
              { label: 'Total Courses', value: '1,200+', icon: <BookOpen className="w-6 h-6 text-cyan-400" /> },
              { label: 'Expert Mentors', value: '300+', icon: <Star className="w-6 h-6 text-amber-400" /> },
              { label: 'Satisfaction Rate', value: '99%', icon: <CheckCircle className="w-6 h-6 text-green-400" /> },
            ].map((stat, i) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {isLoading ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} className="glass rounded-3xl border border-white/10 h-[400px] animate-pulse bg-white/5" />
              ))
            ) : courses.length > 0 ? (
              courses.map((course) => (
              ...
              <div className="col-span-full py-20 text-center">
                <p className="text-slate-500 italic">No courses available at the moment. Check back later!</p>
              </div>
            )}
          </div>
        </div>
              ...
              <div className="flex flex-col lg:flex-row items-center justify-between gap-12 relative z-10">
                <div className="flex-1 text-center lg:text-left">
                  ...
                  <p className="text-xl text-slate-400 mb-10 max-w-xl leading-relaxed">
                    Join our mission to empower learners worldwide. Share your expertise, grow your brand, and earn while making an impact.
                  </p>
                  <a 
                ...
                <div className="flex-1 grid grid-cols-2 gap-4 md:gap-8">
                  {[
                    { label: 'Revenue Share', value: 'Competitive', icon: <Star className="w-5 h-5 text-amber-400" /> },
                    { label: 'Global Students', value: '15k+', icon: <Users className="w-5 h-5 text-indigo-400" /> },
                    { label: 'Teaching Tools', value: 'Pro Suite', icon: <BookOpen className="w-5 h-5 text-cyan-400" /> },
                    { label: 'Support', value: '24/7', icon: <CheckCircle className="w-5 h-5 text-green-400" /> },
                  ].map((item, i) => (
                    <div key={i} className="bg-white/5 border border-white/5 p-6 rounded-3xl hover:bg-white/10 transition-all duration-300">
                      <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center mb-4">{item.icon}</div>
