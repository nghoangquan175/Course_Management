...
export const ExamPlayer: React.FC = () => {
    ...
    const fetchExam = async () => {
      try {
        ...
        if (data.lesson?.course?.lessons) {
          setCourseLessons(
            data.lesson.course.lessons.sort((a: any, b: any) => (a.order || 0) - (b.order || 0))
          );
        }
  ...
  const handleSubmit = React.useCallback(async () => {
    ...
    try {
      ...
      // Notify parent if needed (refresh progress)
      const id = new URLSearchParams(location.search).get('courseId');
      if (id) {
        // You might want to trigger a progress refresh here
      }
    } catch (error) {
  ...
  }, [exam, answers, isSubmitting, location.search]);
  ...
  }, [showStartModal, timeLeft, result, handleSubmit]);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-6">
        <div className="bg-slate-900 border border-white/10 rounded-[2.5rem] p-10 max-w-xl w-full text-center shadow-2xl animate-in zoom-in duration-300">
          ...
          <p className="text-slate-400 mb-8 leading-relaxed">
            {exam.description ||
              `This quiz has ${questions.length} questions. You need ${exam.passingScore}% to pass.`}
          </p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10 text-left">
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
              ...
              <p className="font-bold">
                {exam.timeLimit > 0 ? `${exam.timeLimit} Minutes` : 'No Limit'}
              </p>
            </div>
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
  ...
  if (result && !isReviewMode) {
    ...
    const isCourseFinished =
      courseLessons.length > 0 && courseLessons.every((l) => completedLessonIds.has(l.id));

    const currentIdx = courseLessons.findIndex((l) => l.id === lessonId);
    const nextLesson =
      currentIdx !== -1 && currentIdx < courseLessons.length - 1
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950 p-6 overflow-y-auto">
        <div className="bg-slate-900 border border-white/10 rounded-[3rem] p-12 max-w-2xl w-full text-center shadow-2xl animate-in zoom-in duration-500">
          <div
            className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8 ${result.isPassed ? 'bg-green-500/20 text-green-500' : 'bg-red-500/20 text-red-500'}`}
          >
            {result.isPassed ? (
          ...
          <div className="bg-white/5 rounded-3xl p-8 mb-10 border border-white/5 grid grid-cols-2 gap-8">
            <div>
              ...
              <p
                className={`text-4xl font-black ${result.isPassed ? 'text-green-500' : 'text-red-500'}`}
              >
                {result.score}%
              </p>
            </div>
      ...
      <header className="h-20 shrink-0 border-b border-white/10 bg-slate-900/50 flex items-center justify-between px-10">
        <div className="flex items-center gap-6">
          ...
          <h1 className="text-xl font-bold">
            {isReviewMode ? `Review: ${exam.title}` : exam.title}
          </h1>
        </div>
        ...
        <div className="flex items-center gap-10">
          {!isReviewMode && timeLeft !== null && (
            <div
              className={`flex items-center gap-3 px-6 py-2.5 rounded-2xl border ${timeLeft < 60 ? 'bg-red-500/10 border-red-500 text-red-500 animate-pulse' : 'bg-white/5 border-white/10 text-slate-300'}`}
            >
                      ...
                      return (
                        <div key={optIdx} className="relative">
                          ...
                          >
                            ...
                            <span
                              className={`text-sm font-medium ${isSelected || (isReviewMode && isCorrect) ? 'text-white' : 'text-slate-400'}`}
                            >
                              {opt}
                            </span>
                          </button>
                          {isReviewMode && isCorrect && !isSelected && (
                            <span className="absolute -right-4 top-1/2 -translate-y-1/2 text-[10px] font-bold text-green-500 bg-green-500/10 px-2 py-1 rounded-md">
                              CORRECT ANSWER
                            </span>
                          )}
                        </div>
            ...
            <div className="grid grid-cols-6 gap-2">
              {questions.map((q: any, idx: number) => (
                <button
                  ...
                  className={`aspect-square rounded-lg flex items-center justify-center font-black text-[14px] transition-all border ${
