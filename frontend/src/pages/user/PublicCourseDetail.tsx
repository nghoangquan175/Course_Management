...
export const PublicCourseDetail: React.FC = () => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        if (data.lessons && data.lessons.length > 0) {
          // Group lessons by some logic if needed, for now just show them
        }
      } catch (error) {
  ...
  const handleEnroll = async () => {
    if (!user) {
      toast.error('Please login to join the course');
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 pt-12 pb-20">
      ...
      <div className="container mx-auto px-6 max-w-[1440px]">
        <nav className="flex items-center gap-2 text-sm text-slate-500 mb-8">
          ...
          <ChevronRight className="w-4 h-4" />
          <span className="text-slate-300 truncate">{course.category?.name || 'Uncategorized'}</span>
        </nav>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="flex-1">
            ...
            <p className="text-lg text-slate-400 mb-8 leading-relaxed max-w-3xl">
              {course.description || "Khóa học chuyên sâu giúp bạn làm chủ các kỹ năng thực tế và tư duy đột phá trong lĩnh vực này."}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-6 mb-12">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-indigo-400">{course.instructor?.name || 'Expert Instructor'}</span>
              </div>
            </div>
                ...
                <div className="overflow-hidden">
                  {course.lessons && course.lessons.length > 0 ? (
                    course.lessons.map((lesson: any, idx: number) => (
                      <div key={lesson.id} className="p-5 flex items-center justify-between border-b border-white/5 last:border-0 hover:bg-white/[0.02] transition-colors group">
                        <div className="flex items-center gap-4">
                          <PlayCircle className="w-4 h-4 text-slate-500 group-hover:text-indigo-400 transition-colors" />
                          <div>
                            <p className="text-sm font-medium">Lesson {idx + 1}: {lesson.title}</p>
            ...
            <div className="sticky top-32 glass rounded-[2.5rem] border border-white/10 p-4 shadow-2xl overflow-hidden group">
              ...
              <div className="relative aspect-video rounded-[2rem] overflow-hidden mb-6 bg-slate-900 border border-white/5">
                {isPreviewPlaying ? (
                  <video 
                    ...
                    autoPlay
                  >
                    <source src={getStreamingUrl(course.lessons?.[0]?.videoUrl)} type="application/x-mpegURL" />
                  </video>
                ) : (
                  <>
                    {course.thumbnailUrl ? (
                    ...
                    <div className="absolute bottom-4 left-0 right-0 text-center">
                      <span className="text-xs font-bold text-white drop-shadow-md">Watch Preview</span>
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="px-4 pb-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <span className="px-2 py-1 bg-red-500/20 text-red-500 text-[10px] font-bold rounded-lg uppercase">Sale</span>
                </div>
                ...
                >
                  <span className="text-lg">
                    {course.userEnrollment 
                      ? (course.userEnrollment.status === 'COMPLETED' ? 'COMPLETED' : 'GO TO COURSE') 
                      : 'ENROLL NOW'}
                  </span>
                  {course.userEnrollment?.status === 'COMPLETED' && (
                ...
                <p className="text-center text-[10px] text-slate-500 font-medium">Start learning for free</p>
